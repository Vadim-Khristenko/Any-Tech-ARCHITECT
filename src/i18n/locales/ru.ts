...
export const ru = {
  ...
  "gen.narrowH.detail": "В AmneziaWG 3.1 широкие диапазоны H1–H4 (до 100M) заставляют amneziawg-go 3.1 тратить заметно больше CPU на классификацию пакетов и при включённой HeaderProtection могут давать misclassify из-за пересечения интервалов. Включение сужает каждый H-диапазон до ~20k (H4 ~30k) — фиксит баг ценой чуть меньшей обфускации заголовков. Показывается только для 3.1 и когда включена HeaderProtection (триггер бага). Выкл. по умолчанию.",
