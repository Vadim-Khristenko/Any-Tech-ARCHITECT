...
export const ru = {
  ...
  "gen.narrowH.label": "Уменьшить разброс H1–H4",
  "gen.narrowH.detail": "Важно для 3.1: широкие H1–H4 (до 100M) заставляют amneziawg-go тратить заметно больше CPU на классификацию пакетов, а при включённой HeaderProtection дают misclassify из-за пересечения интервалов. Переключатель сужает каждый диапазон до ~20k (H4 ~30k) и фиксит баг ценой чуть меньшей обфускации заголовков. Выкл. по умолчанию.",
  "gen.narrowH.help": "Широкие H1–H4 в 3.1 — баг: Go-клиент сканирует большие интервалы заголовков, что даёт всплеск CPU и редкие падения рукопожатия при HeaderProtection. Узкие диапазоны (~20k) убирают проблему, но делают заголовки чуть менее разнообразными.",
  ...
  "gen.sameS.label": "Одинаковые S1–S4",
  "gen.sameS.hint":
    "Режим из рекомендаций: одно значение на все четыре S. Мы не рекомендуем: одинаковые S у всех, кто последовал совету, это общий отпечаток. Случайный хвост размывает длины и гасит урон, без хвостов оставляйте разные.",
  ...
  "gen.links.clientFields": "Не знаете, куда вставлять параметры в приложении Amnezia? Жми сюда!",
  ...
  "xg.out.needAddress": "Чтобы собрать ссылку для клиента, укажите адрес сервера в разделе «Ваш сервер».",
  "xg.clients.emailNote": "Email для 3x-ui в панельный экспорт добавляется автоматически — первые 8 символов id, уникально в инбаунде. Вручную вводить не нужно.",
  ...
  "awg3.hpk.switch": "Защита заголовков",
  "awg3.hpk.desc":
  ...
  "awg3.cpa.switch": "Добавочный паддинг",
  "awg3.trailers.switch": "Случайные хвосты",
  "awg3.cookies.switch": "Отключить cookie",
  "awg3.cpa.desc":
  ...
  "conf.awg3HpkManaged":
    "Строки HeaderProtectionKey здесь нет: ключом управляет приложение, включается его переключателем и подхватывается при импорте .conf, поэтому S1–S4 выше держатся на 12+ для нонса шифра",
...
"RandomTrailers/DisableCookies понимает только AWG 3.1; устройство на версии {version} отвергнет эти ключи при чтении конфига.",
  ...
  "find.awg.s_small_managed":
    "{name}={value} < {min}. Ключом управляет приложение: если защита в нём включена, устройство отвергнет конфиг с ошибкой Invalid argument. Поднимите {name} до {min} или выключите защиту заголовков в приложении.",
  ...
  "clientFields.group.awg3": "Параметры 3.x",
  ...
  "clientFields.hint.hpk": "общий ключ шифрования заголовков; в Amnezia VPN генерируется приложением",
  "clientFields.hint.cpa": "случайный паддинг каждого транспортного пакета",
  "clientFields.hint.timer": "диапазон времени протокола",
  "clientFields.hint.trailers": "случайный хвост каждого пакета, только 3.1",
  "clientFields.hint.cookies": "отказ от cookie-ответов, только 3.1",
